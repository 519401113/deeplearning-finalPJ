...
writer = SummaryWriter(log_dir='log/with_pretrained')
step=0
for epoch in tqdm(range(1, n_epochs+1)):
    ...
    # 训练集的模型 #
    ###################
    model.load_state_dict(torch.load('checkpoint/resnet18_contrast.pt'))
    ...
    for data, target in train_loader:
        ...
        output = model(data, frozen=True).to(device)  #（等价于output = model.forward(data).to(device) ）
    ...
    if valid_loss <= valid_loss_min:
        ...
        torch.save(model.state_dict(), 'checkpoint/train_pretrained.pt')
